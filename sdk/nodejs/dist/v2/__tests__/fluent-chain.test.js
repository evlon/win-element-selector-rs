...
describe('FluentChain', () => {
    ...
    beforeEach(() => {
        ...
        chain = new fluent_chain_1.FluentChain(mockClient);
    });
    describe('链式方法', () => {
    ...
    describe('window()', () => {
        it('应该构建正确的窗口选择器', async () => {
            ...
            await chain.run();
            expect(mockClient.activateWindow).toHaveBeenCalledWith("Window[@Name='Test Window' and @ClassName='TestClass' and @ProcessName='TestApp']");
        });
        it('应该接受字符串选择器', async () => {
            mockClient.activateWindow.mockResolvedValue({ success: true });
            chain.window("Window[@ClassName='Test']");
            await chain.run();
            expect(mockClient.activateWindow).toHaveBeenCalledWith("Window[@ClassName='Test']");
        });
    });
    describe('find()', () => {
    ...
    describe('click()', () => {
        it('应该点击找到的元素', async () => {
            ...
            chain
                ...
                .click();
            await chain.run();
            expect(mockClient.moveMouse).toHaveBeenCalledWith({ x: 252, y: 228 }, expect.objectContaining({
                humanize: false,
    ...
    describe('type()', () => {
        it('应该调用 typeText API', async () => {
            ...
            chain
                ...
                .type('test');
            await chain.run();
            expect(mockClient.typeText).toHaveBeenCalledWith('test', expect.objectContaining({
...
//# sourceMappingURL=fluent-chain.test.js.map