...
export class Chain {
     ...
     * 断言元素存在
     */
    async assertExists(xpath: string): Promise<this> {
        if (!this.currentWindowSelector) {
            throw new Error('必须先调用 window() 激活窗口');
        }
        ...
        if (!result.found) {
            await this.failWithScreenshot(
                `Assertion failed: element does not exist: ${xpath}`,
                'assertExists',
                { windowSelector: this.currentWindowSelector, xpath }
            );
        }
     ...
     * 断言元素不存在
     */
    async assertNotExists(xpath: string): Promise<this> {
        if (!this.currentWindowSelector) {
            throw new Error('必须先调用 window() 激活窗口');
        }
        ...
        if (result.found) {
            await this.failWithScreenshot(
                `Assertion failed: element exists when it should not: ${xpath}`,
                'assertNotExists',
                { windowSelector: this.currentWindowSelector, xpath }
            );
        }
     ...
     * 断言元素文本内容
     */
    async assertText(xpath: string, expectedText: string): Promise<this> {
        if (!this.currentWindowSelector) {
            throw new Error('必须先调用 window() 激活窗口');
        }
        ...
        if (!result.found || !result.element) {
            await this.failWithScreenshot(
                `Assertion failed: element not found: ${xpath}`,
                'assertText',
                { windowSelector: this.currentWindowSelector, xpath }
            );
        }
        ...
        if (actualText !== expectedText) {
            await this.failWithScreenshot(
                `Assertion failed: text mismatch. Expected "${expectedText}", got "${actualText}"`,
                'assertText',
                { windowSelector: this.currentWindowSelector, xpath }
            );
        }
     ...
     * 断言元素可见
     */
    async assertVisible(xpath: string): Promise<this> {
        if (!this.currentWindowSelector) {
            throw new Error('必须先调用 window() 激活窗口');
        }
     ...
     * 断言元素可用
     */
    async assertEnabled(xpath: string): Promise<this> {
        if (!this.currentWindowSelector) {
            throw new Error('必须先调用 window() 激活窗口');
        }
        ...
        if (!result.element!.isEnabled) {
            await this.failWithScreenshot(
                `Assertion failed: element is not enabled: ${xpath}`,
                'assertEnabled',
                { windowSelector: this.currentWindowSelector, xpath }
            );
        }
     ...
     */
    async exists(xpath: string): Promise<boolean> {
        if (!this.currentWindowSelector) {
            throw new Error('必须先调用 window() 激活窗口');
        }
     ...
     */
    async tryFind(xpath: string): Promise<ElementInfo | null> {
        if (!this.currentWindowSelector) {
            throw new Error('必须先调用 window() 激活窗口');
        }
