...
pub struct State {
    ...
    pub optimization_cancel_flag: Option<std::sync::Arc<AtomicBool>>,
            ...
            if let Ok(result) = rx.try_recv() {
                if let Some(start_time) = self.optimization_start_time.take() {
                    ...
                    match result {
                        ...
                        None => {
                            let was_cancelled = self.optimization_cancel_flag.as_ref()
                                .map(|f| f.load(Ordering::SeqCst))
                                .unwrap_or(false);
                            let reason = if was_cancelled { "用户取消" } else { "验证超时或失败" };
                            self.status_msg = format!(
                                "✗ 极简优化中止（{:.1}s）：{}，当前结果已保留",
                                elapsed.as_secs_f32(), reason
                            );
                            // Mark in-progress steps as failed
                            use element_selector::core::model::OptimizationStepStatus;
                            for step in &mut self.optimization_steps {
                                if matches!(step.status, OptimizationStepStatus::InProgress) {
                                    step.status = OptimizationStepStatus::Failed(String::from(reason));
                                }
                            }
                        }
                    }
                    ...
                    self.optimization_cancel_flag = None;
                }
            } else {
                self.optimization_rx = Some(rx);
            }
...
pub fn update(state: &mut State, message: Message) -> Task<Message> {
    match message {
        ...
        Message::CancelOptimize => {
            if let Some(ref flag) = state.optimization_cancel_flag {
    ...
    if state.optimization_in_progress.load(Ordering::SeqCst) || !state.optimization_steps.is_empty() {
        ...
        for step in &state.optimization_steps {
            let icon = match &step.status {
                ...
                element_selector::core::model::OptimizationStepStatus::Failed(_) => "✗",
            };
