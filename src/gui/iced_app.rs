...
pub struct State {
    ...
    pub xpath_content: text_editor::Content,
    ...
    pub result_index: String,       // "1-based index, empty = no index"
...
impl State {
    pub fn init() -> (Self, Task<Message>) {
        ...
        let (hierarchy, selected_node, xpath_text, window_selector, element_xpath, window_info, xpath_source, window_filters_enabled, result_index, load_source) =
            if let Some(json) = persistence::load_capture_json() {
                match serde_json::from_str::<PersistedCapture>(&json) {
                    Ok(c) => {
                        ...
                        (result.0, result.1, result.2, result.3, result.4, result.5, result.6, result.7, result.8, "loaded")
                    }
                    Err(_) => {
                        let result = Self::mock_capture();
                        (result.0, result.1, result.2, result.3, result.4, result.5, result.6, result.7, String::new(), "corrupt")
                    }
                }
            } else {
                let result = Self::mock_capture();
                (result.0, result.1, result.2, result.3, result.4, result.5, result.6, result.7, String::new(), "empty")
            };
        ...
        let app = Self {
            ...
            xpath_content: text_editor::Content::new(),
    ...
    fn mock_capture() -> (Vec<HierarchyNode>, Option<usize>, String, String, String, Option<WindowInfo>, XPathSource, Vec<bool>, String) {
        ...
        (result.hierarchy, Some(3), xpath, xpath_result.window_selector, xpath_result.element_xpath, window_info, XPathSource::AutoGenerated, Vec::new(), String::new())
    }

    fn restore_from_persisted(c: PersistedCapture) -> (Vec<HierarchyNode>, Option<usize>, String, String, String, Option<WindowInfo>, XPathSource, Vec<bool>, String) {
        ...
        (hierarchy, c.selected_node, xpath_text, window_selector, element_xpath, window_info, xpath_source, c.window_filters_enabled, result_index)
    }
    ...
    fn complete_capture(&mut self) {
        if let CaptureState::WaitingClick { .. } = self.capture_state {
            ...
            if let Some(cached) = self.cached_hover_result.take() {
                log::info!("[完成捕获] 使用悬停缓存 at ({}, {})", cached.cursor_x, cached.cursor_y);
                self.finish_capture_at(cached.cursor_x, cached.cursor_y);
            } else {
                // 无缓存时，直接用当前鼠标坐标捕获（走底层 UIA，不经过 COM worker）
                let (mx, my, _) = input_hook::get_mouse_state();
                log::info!("[完成捕获] 无缓存，直接用坐标 ({}, {})", mx, my);
                highlight::hide();
                if let Some(ref mut manager) = self.multi_highlight_manager {
                    manager.clear();
                }
                ...
                let result = match self.capture_mode {
                    CaptureMode::Fast => element_selector::core::uia::capture_at_point(mx, my),
                    CaptureMode::Full => element_selector::core::uia::capture_enhanced_at_point(mx, my),
                };
    ...
    fn finish_capture_at(&mut self, x: i32, y: i32) {
        ...
        let result = if let Some(cached) = self.cached_hover_result.take() {
            ...
            match self.capture_mode {
                CaptureMode::Fast => capture::capture_at(x, y),
                CaptureMode::Full => capture::capture_enhanced_at(x, y),
            }
        };
    ...
    fn generate_typescript_code(&self) -> String {
        ...
        match self.code_format {
            ...
            CodeFormat::XPathOnly => element_xpath,
        }
    }
...
pub fn update(state: &mut State, message: Message) -> Task<Message> {
    match message {
        ...
        Message::ResultIndexChanged(val) => {
            state.result_index = val;
            state.xpath_text = format!("{}, {}", state.window_selector, state.final_element_xpath());
            state.xpath_error = xpath::lint(&state.xpath_text);
            state.validation = ValidationResult::Idle;
        }
        Message::XPathEditToggled => {
            state.xpath_edit_mode = !state.xpath_edit_mode;
            if state.xpath_edit_mode {
                // Sync element_xpath into editor content when entering edit mode
        ...
        Message::FilterOperatorChanged(node_idx, filter_idx, op) => {
            if node_idx < state.hierarchy.len() && filter_idx < state.hierarchy[node_idx].filters.len() {
                let filter = &mut state.hierarchy[node_idx].filters[filter_idx];
                // NotEquals/Equals with empty value is meaningful ("attr not empty"/"attr is empty"), auto-enable
                if filter.value.is_empty() && matches!(op, Operator::NotEquals | Operator::Equals) {
                    filter.enabled = true;
                }
                filter.operator = op;
                state.rebuild_xpath();
        ...
        Message::FilterValueChanged(node_idx, filter_idx, val) => {
            if node_idx < state.hierarchy.len() && filter_idx < state.hierarchy[node_idx].filters.len() {
                let filter = &mut state.hierarchy[node_idx].filters[filter_idx];
                filter.value = val;
                // Keep enabled for NotEquals/Equals even with empty value
                if filter.value.is_empty() && !matches!(filter.operator, Operator::NotEquals | Operator::Equals) {
        ...
        Message::WindowFilterOperatorChanged(idx, op) => {
            if idx < state.window_filters.len() {
                let filter = &mut state.window_filters[idx];
                if filter.value.is_empty() && matches!(op, Operator::NotEquals | Operator::Equals) {
                    filter.enabled = true;
                }
                filter.operator = op;
        ...
        Message::XPathEdited(action) => {
            state.xpath_content.perform(action);
            // Sync back to element_xpath
            let new_text = state.xpath_content.text().to_string();
            if new_text != state.element_xpath {
                state.element_xpath = new_text;
                state.xpath_text = format!("{}, {}", state.window_selector, state.final_element_xpath());
                state.xpath_error = xpath::lint(&state.xpath_text);
                state.validation = ValidationResult::Idle;
                state.xpath_source = XPathSource::Manual;
...
fn view_top_bar(state: &State) -> Element<'_, Message> {
    ...
    let complete_capture_btn = if matches!(&state.capture_state, CaptureState::WaitingClick { .. }) && !state.similar_samples.is_empty() {
        let n = state.similar_samples.len();
        Some(
            button(text(format!("完成捕获 ({} 样本)", n)).color(Color::BLACK))
                .padding([4, 12])
                .style(|_, _| button::Style {
                ...
                .on_press(Message::CompleteCapture),
        )
    } else {
        None
    };
...
fn view_xpath_frame(state: &State) -> Element<'_, Message> {
    ...
    // Edit toggle button
    let edit_label = if state.xpath_edit_mode { "查看" } else { "编辑" };
    let edit_btn = button(text(edit_label).size(10)).padding([2, 6])
        .on_press(Message::XPathEditToggled);
    top_items.push(edit_btn.into());
    ...
    // XPath text area: toggle between read-only display and editable input
    let result_index_input = text_input("序号(空=全部)", &state.result_index)
        .size(11)
        .padding([4, 8])
        .width(Length::Fixed(110.0))
        .on_input(Message::ResultIndexChanged);
    let result_index_row = row![
        text("结果序号").size(11).color(colors.muted),
        result_index_input,
    ]
    .spacing(4)
    .align_y(Alignment::Center);
