            ...
            .show_inside(ui, |ui| {
                ui.horizontal(|ui| {
                    ...
                    ui.with_layout(Layout::right_to_left(Align::Center), |ui| {
                        ui.push_id("top_bar_buttons", |ui| {
                            ui.push_id("log_toggle", |ui| {
                            ...
                            match &self.capture_state {
                                CaptureState::WaitingClick { deadline } => {
                                    ...
                                    ui.push_id("cancel_capture_btn", |ui| {
                                        if ui.add(
                                            egui::Button::new(
                                ...
                                CaptureState::Capturing => {
                                    ui.push_id("capturing_indicator", |ui| {
                                        ui.add(
...
CaptureState::Idle => {
                                    ui.push_id("start_capture_btn", |ui| {
                                        if ui.add(
                                            ...
                                            .min_size(Vec2::new(100.0, 28.0)),
                                        ).on_hover_text("F4 — 进入捕获模式").clicked() {
                                            self.start_capture();
                                        }
                                    });
                                }
        ...
        egui::Panel::bottom("bottom_panel")
            ...
            .show_inside(ui, |ui| {
                ...
                ui.horizontal(|ui| {
                    ...
                    ui.with_layout(Layout::right_to_left(Align::Center), |ui| {
                        ui.push_id("bottom_btns", |ui| {
                            ui.push_id("confirm_btn_area", |ui| {
                                let (confirm_text, confirm_fill) = match &self.validation {
                            ...
                            ui.push_id("history_combo_area", |ui| {
                                    ...
                                    egui::ComboBox::from_id_salt("history_combo")
                                        ...
                                        .show_ui(ui, |ui| {
                                            ...
                                            for (i, h) in self.history.iter().enumerate() {
                                                ui.push_id(format!("hist_item_{}", i), |ui| {
    ...
    fn draw_xpath_preview_content(&mut self, ui: &mut Ui) {
        let t = self.theme;
        ui.push_id("xpath_preview_frame", |ui| {
            Frame::NONE
                .fill(t.preview_bg)
    ...
    fn draw_element_xpath_content(&mut self, ui: &mut Ui) {
        let t = self.theme;
        ui.horizontal(|ui| {
            ui.push_id("xpath_btn_row", |ui| {
                ui.label(RichText::new("元素 XPath:").color(t.muted).size(11.0));
                ui.add_space(4.0);

                if ui.add(egui::Button::new("复制").small()).on_hover_text("复制元素 XPath").clicked() {
                    ui.ctx().copy_text(self.element_xpath.clone());
                    self.status_msg = "元素 XPath 已复制到剪贴板".to_string();
                }

                if ui.add(egui::Button::new("智能优化").small()).on_hover_text("自动优化 XPath，移除动态属性，使用锚点定位").clicked() {
                    self.do_optimize();
                }
                ...
                ui.push_id("minimal_btn", |ui| {
                    ...
                    if ui.add(egui::Button::new(btn_text).small()).on_hover_text(btn_tooltip).clicked() {
                        if is_optimizing {
                ...
                if ui.add(egui::Button::new("生成 TS 代码").small()).on_hover_text("生成 TypeScript SDK 选择器代码").clicked() {
                    self.open_code_dialog();
                }

                ui.push_id("reset_btn", |ui| {
                    if !self.xpath_source.is_auto() {
                        if ui.add(egui::Button::new("重置").small()).on_hover_text("恢复为自动生成的 XPath").clicked() {
        ...
        ui.push_id("xpath_edit_area", |ui| {
            let edit_resp = ui.add(
                TextEdit::multiline(&mut self.element_xpath)
                    .id_salt("element_xpath_edit")
    ...
    fn draw_window_xpath_content(&mut self, ui: &mut Ui) {
        let t = self.theme;
        ui.horizontal(|ui| {
            ui.push_id("win_xpath_btn_row", |ui| {
                ui.label(RichText::new("窗口选择器:").color(t.muted).size(11.0));
                ui.add_space(4.0);

                if ui.add(egui::Button::new("复制").small()).on_hover_text("复制窗口选择器").clicked() {
                    ui.ctx().copy_text(self.window_selector.clone());
                    self.status_msg = "窗口选择器已复制到剪贴板".to_string();
                }

                ui.push_id("win_reset_btn", |ui| {
                    if self.custom_window_xpath {
                        if ui.add(egui::Button::new("重置").small()).on_hover_text("回到自动生成的窗口选择器").clicked() {
        ...
        ui.push_id("win_xpath_edit_area", |ui| {
            let edit_resp = ui.add(
                TextEdit::multiline(&mut self.window_selector)
                    .id_salt("window_xpath_edit")
                    .font(egui::TextStyle::Monospace)
                    .desired_rows(2)
                    .desired_width(ui.available_width())
    ...
    /// 校验详情（失败时展示逐段分析）
    fn draw_validation_details(&self, ui: &mut Ui) {
        ui.push_id("validation_details_block", |ui| {
            ...
            ui.push_id("val_result_frame", |ui| {
                egui::Frame::NONE
                    .fill(frame_fill)
                    .corner_radius(CornerRadius::same(4))
                    .inner_margin(Margin::symmetric(10, 6))
...
pub fn draw_tree_flat(
    ...
    for idx in 0..n {
        ui.push_id(format!("tree_node_{}", idx), |ui| {
