...
impl eframe::App for SelectorApp {
    ...
    fn ui(&mut self, ui: &mut Ui, _frame: &mut eframe::Frame) {
        ...
        // Esc 取消捕获（与 UI 提示一致）
        let esc = ui.ctx().input(|i| i.key_pressed(Key::Escape));
        if esc && self.capture_state != CaptureState::Idle {
