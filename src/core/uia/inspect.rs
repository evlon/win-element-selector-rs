    ...
    if is_child_mode {
        ...
        for window in &windows {
            ...
            for child_hwnd in &child_hwnds {
                ...
                if let Ok((elements, _)) = execute_xpath_steps_filtered(&auto, &child_elem, &child_xpath, &FindAllFilter::default(), Some(5000), true) {
        ...
        for window in &windows {
            if let Ok((elements, _)) = execute_xpath_steps_filtered(&auto, window, element_xpath, &FindAllFilter::default(), Some(5000), true) {
                if let Some(elem) = elements.into_iter().next() {
                    target_element = Some(elem);
                    break;
                }
